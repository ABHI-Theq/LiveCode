...
type TemplateSelectionModelProps = {
    ...
    onSubmit: (data: {
        title: string,
        template: "REACTJS" | "NEXTJS" | "EXPRESS" | "HONO" | "VUE" | "ANGULAR",
        description?: string
    }) => void
}
...
const TemplateSelectionModel = ({ isOpen, onClose, onSubmit }: TemplateSelectionModelProps) => {
    ...
    const filteredTemplates = templates.filter((template) => {
        ...
        const matchedSearch = template.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            template.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
            template.tags.some(tag => tag.toLowerCase().includes(searchQuery.toLowerCase()))

        const matchedCategory = category === "all" || template.category === category
    ...
    if (selectedTemplate) {
      const templateMap: Record<
        string,
        "REACTJS" | "NEXTJS" | "EXPRESS" | "VUE" | "HONO" | "ANGULAR"
      > = {
        reactjs: "REACTJS",
        nextjs: "NEXTJS",
        express: "EXPRESS",
                        ...
                        <>
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-bold text-[#e93f3f] flex items-center gap-2">
                                    <Plus className="size-24 text-[#e93f3f]" />
                                    Select a Template
                                </DialogTitle>
                                <DialogDescription>
                            ...
                            <div className="flex flex-col gap-6 py-4">
                                ...
                                <RadioGroup value={selectedTemplate || ""} onValueChange={handleSelectedTemplate}>
